...
- `cashDelta`、`healthDelta`、`luckDelta` 表示基础增减值；`marketPriceMultiplier` 表示商品价格倍率；`grantedQuantity` 表示最多赠送数量；`workIncomeMultiplier` 表示打工收入倍率。
...
事件采用三个核心随机事件分类，并为六个扩展区域各补充三条专属事件：
...
- 健康和钱财事件直接修改玩家状态；现金、健康和运气不会低于零，运气最高为 100。
- 出售“走私电子烟”每笔交易额外降低 5 点运气，并写入生存日记。
...
| ☐ | 13 | `community-leftovers` | 社区活动收尾 | 菲格罗亚走廊、韩国城、西木区、圣盖博、威斯敏斯特·小西贡 | 免费获得最多 6 份国产辣条；运气 +1 |
...
| 连续风险 | 每次拉皮条运气 −1；连续第 3 天不结算收入，直接触发 `lapd-sting-operation`，该次运气 −8 |
| 钓鱼执法 | 弹窗提示、现金 −$1,000、跳过 2 周；债务与存款照常计息 |
| 中断条件 | 倒卖、投资、离开菲格罗亚或在其他区域打工会清零连续次数 |

## 健康事件（17 条）

17 条健康事件均已接入[原生油画卡片](design/health-events/README.md)：独立插图、事件标题、基础效果、剧情与“知道了”按钮。世界事件同周触发时依次展示；健康归零时先呈现健康卡片再进入结局。
...
| ☐ | 5 | `hillside-dehydration` | 山坡脱水 | 好莱坞、帕萨迪纳·玫瑰碗 | 现金 −8，健康 −6 |
| ☐ | 6 | `cooler-failure` | 冷藏箱断电 | 帕萨迪纳·玫瑰碗、菲格罗亚走廊、好莱坞、英格尔伍德 | 现金 −45，健康 −7 |
| ☐ | 7 | `bike-path-fall` | 自行车道摔倒 | 威尼斯、圣塔莫尼卡、卡尔弗城 | 现金 −35，健康 −6 |
| ☐ | 8 | `rip-current` | 离岸流 | 威尼斯、圣塔莫尼卡 | 健康 −12 |
| ☐ | 9 | `earthquake-jolt` | 地震晃动 | 所有地点 | 现金 −25，健康 −5 |
| ☐ | 10 | `sleep-debt` | 连续缺觉 | 所有地点 | 现金 −7，健康 −4 |
| ☐ | 11 | `seasonal-flu` | 季节性流感 | 所有地点 | 现金 −28，健康 −5 |
| ☐ | 12 | `ding-pang-zi-kitchen-smoke` | 后厨油烟呛到 | 丁胖子广场 | 现金 −18，健康 −4 |
| ☐ | 13 | `san-gabriel-kitchen-burn` | 深夜厨房烫伤 | 圣盖博 | 现金 −45，健康 −6 |
| ☐ | 14 | `rowland-market-back-strain` | 闭店搬货腰伤 | 罗兰岗 | 现金 −25，健康 −5 |
| ☐ | 15 | `industry-warehouse-rush-strain` | 爆单分拣拉伤 | 工业市 | 现金 −30，健康 −7 |
| ☐ | 16 | `irvine-long-commute-fatigue` | 长途通勤透支 | 尔湾 | 现金 −30，健康 −6 |
| ☐ | 17 | `little-saigon-kitchen-slip` | 湿滑后厨摔倒 | 威斯敏斯特·小西贡 | 现金 −35，健康 −5 |
...
| ☐ | 7 | `vending-compliance` | 补齐摆摊手续 | 菲格罗亚走廊、好莱坞、威尼斯、帕萨迪纳·玫瑰碗、丁胖子广场、威斯敏斯特·小西贡 | 现金 −85，运气 +2 |
| ☐ | 8 | `ding-pang-zi-referral-shift` | 熟客介绍短工 | 丁胖子广场 | 现金 +110，运气 +1 |
| ☐ | 9 | `san-gabriel-repaid-tab` | 熟客补回欠款 | 圣盖博 | 现金 +140，运气 +1 |
| ☐ | 10 | `rowland-carpool-delivery` | 拼车送货赚外快 | 罗兰岗 | 现金 +125，健康 −2 |
| ☐ | 11 | `industry-damaged-shipment` | 运输货损赔偿 | 工业市 | 现金 −190，运气 −1 |
| ☐ | 12 | `irvine-expo-overtime` | 科技展会加班费 | 尔湾 | 现金 +180，运气 +1 |
| ☐ | 13 | `little-saigon-wedding-tips` | 婚宴帮工小费 | 威斯敏斯特·小西贡 | 现金 +130，运气 +1 |
| ☐ | 14 | `freeway-gridlock` | 高速堵死 | 所有地点 | 现金 −250；健康不变 |

“高速堵死”剧情：高速堵得水泄不通，聪明的你发现最左侧的 Express Lane 畅通无比，果断驶入，一路飙车。最终，你收获了一张 250 美元的罚单。此事件保留原有油画卡片，分类显示为“钱财事件”。
