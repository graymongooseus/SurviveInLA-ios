...
enum AdventurePurchaseError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .noActiveSession:
            "请先开始一局游戏，再购买奇遇。未完成的 App Store 交易会在下一局开始后自动补发。"
        }
    }
}
...
final class AdventureShopStore {
...
#if DEBUG
        ProcessInfo.processInfo.environment["SURVIVE_IN_LA_DIRECT_GRANT"] == "1"
#else
        false
#endif
    }
    ...
    func processUnfinishedTransactions(
        ...
        guard !usesDirectGrantMode else { return true }
        var handledEveryTransaction = true
        ...
        return handledEveryTransaction
    }
    ...
    func syncPurchases(
        ...
        guard !usesDirectGrantMode else {
            notice = UserNotice(title: "正在使用测试模式", message: "点击商品会直接发放剧情和游戏存款，不会连接 App Store。")
            return
        }

        do {
            try await AppStore.sync()
            if await processUnfinishedTransactions(grant: grant) {
                notice = UserNotice(
                    title: "交易已同步",
                    message: "已检查并补发所有可验证的未完成交易。已发放的消耗型奇遇不会重复恢复。"
                )
            }
        } catch {
