    ...
    private var gameContent: some View {
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                ...
                if let choice = store.pendingLifeChoice {
                    LifeChoiceOverlay(store: store, choice: choice)
                        .transition(.opacity.combined(with: .scale(scale: 0.97)))
                        .zIndex(13)
                }

                if let event = store.lifeChoiceResult {
                ...
                if store.canPresentInvestmentEvent,
                   store.investmentNotice != nil || (!store.session.isFinished && store.session.pendingInvestmentInvitation != nil) {
        ...
        .alert(item: Binding<UserNotice?>(
            get: {
                guard let notice = store.notice,
                      notice.healthEvent == nil,
                      notice.event?.id.hasPrefix("life-choice-") != true else { return nil }
                return notice
            },
            set: { newValue in
                guard store.notice?.healthEvent == nil else { return }
                if newValue == nil {
                    store.dismissNotice()
                } else {
...
private struct LifeChoiceResultOverlay: View {
    ...
    private func resultCard(title: String, value: Int, symbol: String, isCash: Bool = false) -> some View {
        ...
        let color = value < 0 ? AppTheme.coral : (value > 0 ? AppTheme.positive : .white.opacity(0.58))
