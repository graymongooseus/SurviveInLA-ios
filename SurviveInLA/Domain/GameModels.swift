...
struct GameSession: Codable, Sendable {
    ...
    // Stored with the game state so a rewarded StoreKit transaction and its
    // idempotency marker are committed by the same atomic snapshot write.
    var processedPurchaseTransactionIDs: Set<String>? = nil
    ...
    var pendingCelebrityEncounter: PendingCelebrityEncounter? = nil
    var queuedCelebrityEncounters: [PendingCelebrityEncounter]? = nil
    var lastCelebrityArrivalTurn: Int? = nil
    var celebrityPhotos: [CelebrityPhoto]? = nil
    var playerBuffs: [ActivePlayerBuff]? = nil
    var pendingActingEncounter: PendingActingEncounter? = nil
    var queuedActingEncounters: [PendingActingEncounter]? = nil
    var lastActingArrivalTurn: Int? = nil
    var resolvedActingEventIDs: Set<String>? = nil
    var pendingActingBonuses: [ScheduledActingBonus]? = nil
    var unreadActingNotices: [GameEvent]? = nil
    ...
    var currentLuck: Int {
        get { min(100, max(0, baseLuck + playerLuckBonus)) }
        // Direct assignment configures the base stat; temporary bonuses never
        // become permanent when an ordinary event changes luck.
        set { baseLuck = newValue }
    }
