...
struct EventContentManifest: Codable, Sendable {
    ...
    let celebrityFiles: [String]?
    let celebrityPhotoEventFiles: [String]?
    let celebrityActingEventFiles: [String]?
...
struct EventContentCatalog: Sendable {
    ...
    let celebritiesByID: [String: CelebrityDefinition]
    let celebrityPhotoEvents: [CelebrityPhotoEventDefinition]
    let celebrityActingEvents: [CelebrityActingEventDefinition]
    let locationsByID: [String: GameEvent]
    let worldsByID: [String: WorldEvent]
    let choicesByID: [String: LifeChoiceEvent]

    static var bundledContentURL: URL? {
        #if SWIFT_PACKAGE
        Bundle.module.resourceURL?.appendingPathComponent("Content")
        #else
        Bundle.main.resourceURL?.appendingPathComponent("Content")
        #endif
    }

    static let bundled: EventContentCatalog = {
        do {
            guard let root = bundledContentURL else {
    ...
    static func load(from root: URL) throws -> EventContentCatalog {
        ...
        let celebrities: [CelebrityDefinition] = try read(manifest.celebrityFiles ?? [], root: root, strings: strings)
        let photos: [CelebrityPhotoEventDefinition] = try read(manifest.celebrityPhotoEventFiles ?? [], root: root, strings: strings)
        let acting: [CelebrityActingEventDefinition] = try read(manifest.celebrityActingEventFiles ?? [], root: root, strings: strings)
        guard celebrities.allSatisfy({ !$0.id.isEmpty && !$0.title.isEmpty }),
              Set(celebrities.map(\.id)).count == celebrities.count else { throw ContentError("Invalid celebrity registry") }
        let celebritiesByID = Dictionary(uniqueKeysWithValues: celebrities.map { ($0.id, $0) })
        let allIDs = locations.map(\.id) + worlds.map(\.id) + choices.map(\.id) + investments.map(\.id) + photos.map(\.id) + acting.map(\.id)
        ...
        for event in acting {
            guard celebritiesByID[event.sourceNPCID] != nil,
                  (1 ... 51).contains(event.delayTurns), (1 ... 100).contains(event.bonusMultiplier),
                  !event.options.isEmpty, event.options.count <= 8,
                  Set(event.options.map(\.id)).count == event.options.count,
                  event.options.allSatisfy({ !$0.id.isEmpty && (0 ... 1_000_000).contains($0.fee)
                      && (-100 ... 100).contains($0.healthDelta) && (-100 ... 100).contains($0.luckDelta) }) else {
                throw ContentError("\(event.id): invalid acting options or bonus")
            }
            try validate(event.condition, eventID: event.id, worlds: worldsByID, choices: choicesByID)
        }
        for event in photos {
            guard celebritiesByID[event.sourceNPCID] != nil, !event.options.isEmpty, event.options.count <= 8,
                  event.options.allSatisfy({ !$0.id.isEmpty }),
                  Set(event.options.map(\.id)).count == event.options.count else {
                throw ContentError("\(event.id): invalid celebrity or photo choices")
            }
            try validate(event.condition, eventID: event.id, worlds: worldsByID, choices: choicesByID)
            for option in event.options {
                let b = option.buff
                guard (1 ... 1_000_000).contains(option.salePrice), !b.id.isEmpty,
                      (1 ... 52).contains(b.durationTurns), (-100 ... 100).contains(b.luckDelta),
                      (-100 ... 100).contains(b.healthPerTurn), b.workIncomeMultiplier.isFinite,
                      (0 ... 3).contains(b.workIncomeMultiplier) else {
                    throw ContentError("\(event.id)/\(option.id): invalid photo price or personal buff")
                }
            }
        }
        for event in investments {
            guard manifest.celebrityFiles == nil || celebritiesByID[event.sourceNPCID] != nil else {
                throw ContentError("\(event.id): unknown celebrity reference")
            }
        ...
        return EventContentCatalog(
            ...
            celebritiesByID: celebritiesByID, celebrityPhotoEvents: photos, celebrityActingEvents: acting,
    ...
    private static func localize(_ value: Any, strings: [String: String]) throws -> Any {
        ...
        let textFields = ["titleKey": "title", "messageKey": "message", "storyKey": "story", "resultKey": "result", "photoTitleKey": "photoTitle"]
