...
def condition(value, where, world_ids, choices, district_ids, depth=0):
    ...
    if kind in ("all", "any", "not"):
        ...
        require(args["comparison"] in ["equal", "atLeast", "atMost"], where, "unknown comparison")
...
def main():
    ...
    fields(manifest, ["schemaVersion", "contentVersion", "localeFile", "locationFiles", "worldFiles", "lifeChoiceFiles", "investmentEventFiles", "worldSchedule", "lifeChoiceStages"], ["schemaVersion", "contentVersion", "localeFile", "locationFiles", "worldFiles", "lifeChoiceFiles", "worldSchedule", "lifeChoiceStages"], "manifest")
    ...
    entries = []
    for family in ("location", "world", "lifeChoice", "investmentEvent"):
    ...
    def text_keys(entry, where):
        for key, value in entry.items():
            if key in ["titleKey", "messageKey", "storyKey", "resultKey"]:
    ...
    if args.export_catalog:
        ...
        for family, filename, event in entries:
            ...
            title_key = event["invitation"]["titleKey"] if family == "investmentEvent" else event["titleKey"]
            report.append(f"| {event['id']} | {event.get('group', family)} | {strings[title_key]} | {scope} | {detail} | {filename} |")
